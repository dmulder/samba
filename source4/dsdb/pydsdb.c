...
{ 
	...
	sid = dom_sid_parse_talloc(NULL, PyStr_AsUTF8(py_sid));
	if (sid == NULL) {
		PyErr_NoMemory();
		return NULL;
	}
...
static PyObject *py_dsdb_DsReplicaAttribute(PyObject *self, PyObject *args)
{
	...
	if (!PyList_Check(el_list)) {
		...
		for (i = 0; i < el->num_values; i++) {
			...
			el->values[i].data = (uint8_t *)PyStr_AsUTF8(item);
...
static PyObject *py_dsdb_set_ntds_invocation_id(PyObject *self, PyObject *args)
{
	...
	PyErr_LDB_OR_RAISE(py_ldb, ldb);
	GUID_from_string(PyStr_AsUTF8(py_guid), &guid);
...
static PyObject *py_dsdb_garbage_collect_tombstones(PyObject *self, PyObject *args)
{
	...
	for (i = 0; i < length; i++) {
		char *part_str = PyStr_AsUTF8(PyList_GetItem(py_list_dn, i));
		struct ldb_dn *p;
