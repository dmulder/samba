...
struct frstrans_connection {
	...
	struct frstrans_connection *next;
	struct frstrans_connection *prev;
};

static struct frstrans_connection *connections = NULL;

static void remove_frstrans_connection(struct GUID *replica_set_guid,
				    struct GUID *connection_guid)
{
	struct frstrans_connection *connection = connections;
	while(connection != NULL) {
		if (memcmp(replica_set_guid, &(connection->replica_set_guid), sizeof(struct GUID)) == 0 &&
		    memcmp(connection_guid, &(connection->connection_guid), sizeof(struct GUID)) == 0) {
			struct frstrans_connection *prev = connection->prev;
			struct frstrans_connection *next = connection->next;
			if (prev) {
				prev->next = next;
			}
			if (next) {
...
static void push_frstrans_connection(struct GUID *replica_set_guid,
				  struct GUID *connection_guid)
{
	struct frstrans_connection *connection = connections;
	if (connection == NULL) {
		connections = talloc_zero(NULL, struct frstrans_connection);
		connection = connections;
	} else {
		remove_frstrans_connection(replica_set_guid, connection_guid);
		for (; connection->next; connection = connection->next) {}
		connection->next = talloc_zero(NULL, struct frstrans_connection);
		       ...
		       struct frstrans_EstablishConnection *r)
{
	...
	int conn_enabled = 1;
	const char* inbound_partner = NULL, *outbound_partner = NULL;
	static const char *mem_attrs[] = {
		"distinguishedName",
	};
	...
	*/
	...
	ldbret = ldb_search(ldb, mem_ctx, &res, repl_dn, LDB_SCOPE_ONELEVEL, mem_attrs,
	...
	outbound_partner = ldb_msg_find_attr_as_string(res->msgs[0],
						       "distinguishedName",
						       "");
	...
	*/
	if (repl_type != SYSVOL) {
		...
		conn_enabled = ldb_msg_find_attr_as_bool(res->msgs[0],
							 "msDFSR-Enabled",
							 1);
		inbound_partner = ldb_msg_find_attr_as_string(res->msgs[0],
							      "FromServer",
							      "");
	}
	...
	*/
	if (repl_type != SYSVOL) {
		if (strcasecmp(outbound_partner, inbound_partner) != 0) {
			ret = WERR_FRS_ERROR_CONNECTION_INVALID;
			goto out;
		}
	}
	...
	*/
	if (r->in.downstream_protocol_version == 0x00050001 ||
		...
		ret = WERR_FRS_ERROR_INCOMPATIBLE_VERSION;
		goto out;
	}

	push_frstrans_connection(&(r->in.replica_set_guid), &(r->in.connection_guid));
