			       ...
			       struct smb2cli_state *cli)
{
	...
	smb2_transport_credits_ask_num(cli->tree->session->transport, 256);

	smb2_util_unlink(cli->tree, FNAME);

	status = smb2_util_roothandle(cli->tree, &dh);
	CHECK_STATUS(status, NT_STATUS_OK);

	max_buffer_size =
		smb2cli_conn_max_trans_size(cli->tree->session->transport->conn);
	...
	req = smb2_notify_send(cli->tree, &n);
	...
	status = torture_setup_simple_file(torture, cli->tree, FNAME);
	CHECK_STATUS(status, NT_STATUS_OK);
	...
	status = smb2_util_close(cli->tree, dh);
	CHECK_STATUS(status, NT_STATUS_OK);
	status = smb2_util_roothandle(cli->tree, &dh);
	CHECK_STATUS(status, NT_STATUS_OK);
			      ...
			      struct smb2cli_state *cli2)
{
	...
	smb2_deltree(cli1->tree, BASEDIR_DIR);
	smb2_util_rmdir(cli1->tree, BASEDIR_DIR);
	/*
	...
	*/
	...
	req = smb2_notify_send(cli1->tree, &(notify.smb2));
	smb2_util_mkdir(cli2->tree, fname);
	...
	req = smb2_notify_send(cli1->tree, &(notify.smb2));
	smb2_util_rmdir(cli2->tree, fname);
	...
	req2 = smb2_notify_send(cli1->tree, &(notify.smb2));
	...
	status = smb2_util_unlink(cli1->tree, BASEDIR_DIR "\\nonexistent.txt");
	...
	status = smb2_util_unlink(cli2->tree, BASEDIR_DIR "\\test0.txt");
	CHECK_STATUS(status, NT_STATUS_OK);
	...
	for (i=1;i<count;i++) {
		...
		status = smb2_util_unlink(cli2->tree, fname2);
		CHECK_STATUS(status, NT_STATUS_OK);
		talloc_free(fname2);
	}
	...
	smb2_util_close(cli1->tree, h2);
	smb2_deltree(cli1->tree, BASEDIR_DIR);
	return ret;
}

static struct smb2_handle custom_smb2_create(struct smb2cli_state *cli,
						...
						struct smb2_create *smb2)
{
	...
	bool ret = true;
	NTSTATUS status;
	smb2_deltree(cli->tree, smb2->in.fname);
	status = smb2_create(cli->tree, torture, smb2);
	CHECK_STATUS(status, NT_STATUS_OK);
				...
				struct smb2cli_state *cli2)
{
	...
	smb2_deltree(cli1->tree, BASEDIR_REC);
	smb2_util_rmdir(cli1->tree, BASEDIR_REC);
	...
	*/
	...
	notify.smb2.in.recursive = true;
	req1 = smb2_notify_send(cli1->tree, &(notify.smb2));
	smb2_cancel(req1);
	...
	status = smb2_create(cli2->tree, torture, &(io1.smb2));
	CHECK_STATUS(status, NT_STATUS_OK);
	smb2_util_close(cli2->tree, io1.smb2.out.file.handle);
	...
	status = smb2_setinfo_file(cli2->tree, &sinfo);
	CHECK_STATUS(status, NT_STATUS_OK);
	...
	status = smb2_util_rmdir(cli2->tree,
		BASEDIR_REC "\\subdir-name\\subname1-r");
	CHECK_STATUS(status, NT_STATUS_OK);
	status = smb2_util_rmdir(cli2->tree,
		BASEDIR_REC "\\subdir-name");
	CHECK_STATUS(status, NT_STATUS_OK);
	status = smb2_util_unlink(cli2->tree, BASEDIR_REC "\\subname3-r");
	CHECK_STATUS(status, NT_STATUS_OK);
					    ...
					    struct smb2cli_state *cli2)
{
	...
	smb2_deltree(cli1->tree, BASEDIR_MC);
	smb2_util_rmdir(cli1->tree, BASEDIR_MC);
	...
	*/
	...
	smb2_util_close(cli1->tree,
		custom_smb2_create(cli1, torture, &(io1.smb2)));
	status = smb2_util_setatr(cli1->tree, BASEDIR_MC "\\tname1",
				FILE_ATTRIBUTE_HIDDEN);
	CHECK_STATUS(status, NT_STATUS_OK);
	smb2_util_unlink(cli1->tree, BASEDIR_MC "\\tname1");
	...
	status = smb2_util_rmdir(cli2->tree, BASEDIR_MC "\\subdir-name\\subname1-r");
	CHECK_STATUS(status, NT_STATUS_OK);
	status = smb2_util_rmdir(cli2->tree, BASEDIR_MC "\\subdir-name");
	CHECK_STATUS(status, NT_STATUS_OK);
	status = smb2_util_unlink(cli2->tree, BASEDIR_MC "\\subname3-r");
	CHECK_STATUS(status, NT_STATUS_OK);
				     ...
				     struct smb2cli_state *cli2)
{
	...
	smb2_deltree(cli1->tree, BASEDIR_MSK);
	smb2_util_rmdir(cli1->tree, BASEDIR_MSK);
...
#define NOTIFY_MASK_TEST(test_name, setup, op, cleanup, Action, \
	...
	NOTIFY_MASK_TEST("Testing mkdir",;,
			 smb2_util_mkdir(cli2->tree, BASEDIR_MSK "\\tname1");,
			 smb2_util_rmdir(cli2->tree, BASEDIR_MSK "\\tname1");,
			 NOTIFY_ACTION_ADDED,
	...
	NOTIFY_MASK_TEST("Testing create file",;,
			 smb2_util_close(cli2->tree, custom_smb2_create(cli2,
						torture, &(io1.smb2)));,
			 smb2_util_unlink(cli2->tree, BASEDIR_MSK "\\tname1");,
			 NOTIFY_ACTION_ADDED,
	...
	NOTIFY_MASK_TEST("Testing rename file",
			 smb2_util_close(cli2->tree, custom_smb2_create(cli2,
						torture, &(io1.smb2)));,
			 smb2_setinfo_file(cli2->tree, &sinfo);,
			 smb2_util_unlink(cli2->tree, BASEDIR_MSK "\\tname2");,
	...
	NOTIFY_MASK_TEST("Testing rename dir",
		smb2_util_mkdir(cli2->tree, BASEDIR_MSK "\\tname1");,
		smb2_setinfo_file(cli2->tree, &sinfo);,
		smb2_util_rmdir(cli2->tree, BASEDIR_MSK "\\tname2");,
		NOTIFY_ACTION_OLD_NAME,
	...
	NOTIFY_MASK_TEST("Testing set path attribute",
		smb2_util_close(cli2->tree, custom_smb2_create(cli2,
				       torture, &(io.smb2)));,
		smb2_util_setatr(cli2->tree, BASEDIR_MSK "\\tname1",
				 FILE_ATTRIBUTE_HIDDEN);,
		smb2_util_unlink(cli2->tree, BASEDIR_MSK "\\tname1");,
		NOTIFY_ACTION_MODIFIED,
	...
	if (torture_setting_bool(torture, "samba3", false)) {
		...
		NOTIFY_MASK_TEST("Testing set file create time",
			smb2_create_complex_file(torture, cli2->tree,
			BASEDIR_MSK "\\tname1", &h2);,
			smb2_setinfo_file(cli2->tree, &sinfo);,
			(smb2_util_close(cli2->tree, h2),
			 smb2_util_unlink(cli2->tree, BASEDIR_MSK "\\tname1"));,
			NOTIFY_ACTION_MODIFIED,
	...
	NOTIFY_MASK_TEST("Testing write",
		smb2_create_complex_file(torture,
			...
			BASEDIR_MSK "\\tname1",
			&h2);,
		smb2_util_write(cli2->tree, h2, &c, 10000, 1);,
		(smb2_util_close(cli2->tree, h2),
		smb2_util_unlink(cli2->tree, BASEDIR_MSK "\\tname1"));,
		NOTIFY_ACTION_MODIFIED,
		0, 1);
				...
				struct smb2cli_state *cli)
{
	...
	smb2_deltree(cli->tree, BASEDIR_FL);
	smb2_util_rmdir(cli->tree, BASEDIR_FL);

	torture_comment(torture, "TESTING CHANGE NOTIFY ON FILES\n");
	status = torture_smb2_testdir(cli->tree, BASEDIR_FL, &h1);
	CHECK_STATUS(status, NT_STATUS_OK);
	...
	status = smb2_create(cli->tree, torture, &(io.smb2));
	CHECK_STATUS(status, NT_STATUS_OK);
	h1 = io.smb2.out.file.handle;
	...
	req = smb2_notify_send(cli->tree, &(notify.smb2));
	status = smb2_notify_recv(req, torture, &(notify.smb2));
	CHECK_STATUS(status, NT_STATUS_INVALID_PARAMETER);
	...
	status = smb2_close(cli->tree, &(cl.smb2));
	CHECK_STATUS(status, NT_STATUS_OK);
		...
		struct smb2cli_state *cli)
{
	...
	smb2_deltree(cli->tree, BASEDIR_TD);
	smb2_util_rmdir(cli->tree, BASEDIR_TD);
		...
		struct smb2cli_state *cli)
{
	...
	smb2_deltree(cli->tree, BASEDIR_NTDIS);
	smb2_util_rmdir(cli->tree, BASEDIR_NTDIS);
				...
				struct smb2cli_state *cli1)
{
	...
	smb2_deltree(cli1->tree, BASEDIR_CNC);
	smb2_util_rmdir(cli1->tree, BASEDIR_CNC);
				...
				struct smb2cli_state *cli1)
{
	...
	smb2_deltree(cli1->tree, BASEDIR_NUL);
	smb2_util_rmdir(cli1->tree, BASEDIR_NUL);
	...
	*/
	...
	status = smb2_logoff(cli1->tree->session);
	CHECK_STATUS(status, NT_STATUS_OK);
				...
				struct smb2cli_state *cli1)
{
	...
	smb2_deltree(cli1->tree, BASEDIR_NSR);
	smb2_util_rmdir(cli1->tree, BASEDIR_NSR);
	...
	*/
	...
	previous_session_id = smb2cli_session_current_id(cli1->tree->session->smbXcli);
	torture_assert(torture, torture_smb2_session_setup(torture,
		       cli1->tree->session->transport,
		       previous_session_id,
		       torture, &session2),
					       ...
					       struct smb2cli_state *cli2)
{
	...
	smb2_deltree(cli2->tree, BASEDIR_IR);
	smb2_util_rmdir(cli2->tree, BASEDIR_IR);
	...
	*/
	...
	status = smb2_session_setup_spnego(cli1->tree->session,
					   invalid_creds,
...
{
	struct smb2cli_state *cli = (struct smb2_tree *)p;
	smb2_transport_dead(cli->tree->session->transport,
		...
		struct smb2cli_state *cli)
{
	...
	smb2_deltree(cli->tree, BASEDIR_NTCPD);
	smb2_util_rmdir(cli->tree, BASEDIR_NTCPD);
	...
	*/
	...
	notify.smb2.in.recursive = true;
	req = smb2_notify_send(cli->tree, &(notify.smb2));
	smb2_transport_idle_handler(cli->tree->session->transport,
				tcp_dis_handler, 250000, cli->tree);
	cli->tree = NULL;
	status = smb2_notify_recv(req, torture, &(notify.smb2));
			...
			struct smb2cli_state *cli2)
{
	...
	smb2_deltree(cli1->tree, BASEDIR_NDOH);
	smb2_util_rmdir(cli1->tree, BASEDIR_NDOH);
	...
	*/
	...
	smb2_util_mkdir(cli2->tree, BASEDIR_NDOH "\\subdir-name");
	req1 = smb2_notify_send(cli1->tree, &(notify.smb2));
	req2 = smb2_notify_send(cli1->tree, &(notify.smb2));
	...
	smb2_util_mkdir(cli2->tree, BASEDIR_NDOH "\\subdir-name2");
			     ...
			     struct smb2cli_state *cli)
{
	...
	smb2_deltree(cli->tree, BASEDIR_TREE);
	smb2_util_rmdir(cli->tree, BASEDIR_TREE);
	...
	/*
	  setup the directory cli->tree, and the notify buffer on each directory
	*/
	...
	for (i=0;i<ARRAY_SIZE(dirs);i++) {
		char *path = talloc_asprintf(torture, "%s\\test.dir",
					     dirs[i].path);
		smb2_util_mkdir(cli->tree, path);
		smb2_util_rmdir(cli->tree, path);
		talloc_free(path);
	}
	...
	*/
	for (i=ARRAY_SIZE(dirs)-1;i>=0;i--) {
		smb2_util_close(cli->tree, dirs[i].h1);
		smb2_util_rmdir(cli->tree, dirs[i].path);
	}
				...
				struct smb2cli_state *cli)
{
	...
	smb2_deltree(cli->tree, BASEDIR_OVF);
	smb2_util_rmdir(cli->tree, BASEDIR_OVF);
	...
	notify.smb2.in.recursive = true;
	req1 = smb2_notify_send(cli->tree, &(notify.smb2));
	...
	for (i=0;i<count;i++) {
		...
		h2 = custom_smb2_create(cli, torture, &(io1.smb2));
		talloc_free(fname);
				...
				struct smb2cli_state *cli2)
{
	...
	smb2_deltree(cli1->tree, BASEDIR_BAS);
	smb2_util_rmdir(cli1->tree, BASEDIR_BAS);
	...
	status =  smb2_create(cli2->tree, torture, &(io.smb2));
	CHECK_STATUS(status,NT_STATUS_OK);
	smb2_util_close(cli2->tree, io.smb2.out.file.handle);
	...
	smb2_util_setatr(cli2->tree, BASEDIR_BAS, FILE_ATTRIBUTE_HIDDEN);

	/* set attribute on a file to assure we receive a notification */
	smb2_util_setatr(cli2->tree, BASEDIR_BAS "\\tname1", FILE_ATTRIBUTE_HIDDEN);
	smb_msleep(200);
				  ...
				  struct smb2cli_state *cli)
{
	...
	smb2_deltree(cli->tree, BASEDIR_TCON);
	smb2_util_rmdir(cli->tree, BASEDIR_TCON);
	...
	*/
	...
	notify.smb2.in.recursive = true;
	req = smb2_notify_send(cli->tree, &(notify.smb2));
	status = smb2_util_mkdir(cli->tree, fname);
	CHECK_STATUS(status, NT_STATUS_OK);
	...
	torture_comment(torture, "Testing notify rmdir\n");
	req = smb2_notify_send(cli->tree, &(notify.smb2));
	status = smb2_util_rmdir(cli->tree, fname);
	CHECK_STATUS(status, NT_STATUS_OK);
	...
	if (!torture_smb2_tree_connect(torture, cli->tree->session, cli->tree, &cli1->tree)) {
	...
	torture_comment(torture, "tid1=%d tid2=%d\n",
			smb2cli_tcon_current_id(cli->tree->smbXcli),
			smb2cli_tcon_current_id(cli1->tree->smbXcli));

	torture_comment(torture, "Testing notify mkdir\n");
	req = smb2_notify_send(cli->tree, &(notify.smb2));
	smb2_util_mkdir(cli1->tree, fname);
	...
	torture_comment(torture, "Testing notify rmdir\n");
	req = smb2_notify_send(cli->tree, &(notify.smb2));
	smb2_util_rmdir(cli->tree, fname);
	...
	torture_comment(torture, "Disconnecting secondary cli->tree\n");
	status = smb2_tdis(cli1->tree);
	CHECK_STATUS(status, NT_STATUS_OK);
	talloc_free(cli1->tree);

	torture_comment(torture, "Testing notify mkdir\n");
	req = smb2_notify_send(cli->tree, &(notify.smb2));
	smb2_util_mkdir(cli->tree, fname);
				      ...
				      bool initial_delete_on_close)
{
	...
	smb2_deltree(cli1->tree, BASEDIR_RMD);
	smb2_util_rmdir(cli1->tree, BASEDIR_RMD);
	...
	if (initial_delete_on_close) {
		status = smb2_util_rmdir(cli2->tree, BASEDIR_RMD);
		CHECK_STATUS(status, NT_STATUS_OK);
	} else {
		status = smb2_create(cli2->tree, torture, &(io.smb2));
		CHECK_STATUS(status, NT_STATUS_OK);
		...
		status = smb2_setinfo_file(cli2->tree, &sfinfo);
		CHECK_STATUS(status, NT_STATUS_OK);
...
static bool torture_smb2_notify_rmdir1(struct torture_context *torture,
				       struct smb2cli_state *cli)
{
	return torture_smb2_notify_rmdir(torture, cli, cli, false);
}

static bool torture_smb2_notify_rmdir2(struct torture_context *torture,
				       struct smb2cli_state *cli)
{
	return torture_smb2_notify_rmdir(torture, cli, cli, true);
}
...
static bool torture_smb2_notify_rmdir3(struct torture_context *torture,
				       ...
				       struct smb2cli_state *cli2)
{
	return torture_smb2_notify_rmdir(torture, cli1, cli2, false);
}
...
static bool torture_smb2_notify_rmdir4(struct torture_context *torture,
				       ...
				       struct smb2cli_state *cli2)
{
	return torture_smb2_notify_rmdir(torture, cli1, cli2, true);
}
					...
					struct smb2cli_state *cli2)
{
	...
	smb2_deltree(cli1->tree, BASEDIR_INR);

	torture_comment(torture, "Testing change notify of a rename with inotify\n");

	status = torture_smb2_testdir(cli1->tree, BASEDIR_INR, &h1);
	...
	status = smb2_create(cli2->tree, torture, &create);
	...
	req = smb2_notify_send(cli1->tree, &notify);
