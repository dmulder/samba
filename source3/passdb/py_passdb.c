...
static PyObject *py_samu_get_lanman_passwd(PyObject *obj, void *closure)
{
	...
	py_lm_pw = PyStr_FromStringAndSize(lm_pw, LM_HASH_LEN);
	talloc_free(frame);
	return py_lm_pw;
}
...
static PyObject *py_samu_get_nt_passwd(PyObject *obj, void *closure)
{
	...
	py_nt_pw = PyStr_FromStringAndSize(nt_pw, NT_HASH_LEN);
	talloc_free(frame);
	return py_nt_pw;
}
...
static PyObject *py_samu_get_pw_history(PyObject *obj, void *closure)
{
	...
	py_nt_pw_his = PyStr_FromStringAndSize(nt_pw_his, hist_len*PW_HISTORY_ENTRY_LEN);
	talloc_free(frame);
	return py_nt_pw_his;
}
...
static PyObject *py_pdb_get_aliasinfo(PyObject *self, PyObject *args)
{
	...
	PyDict_SetItemString(py_alias_info, "rid",
			     PyLong_FromLong(alias_info->rid));
...
static PyObject *py_pdb_search_users(PyObject *self, PyObject *args)
{
	...
	while (search->next_entry(search, entry)) {
		...
		if (py_dict == NULL) {
			...
			PyDict_SetItemString(py_dict, "idx", PyLong_FromLong(entry->idx));
			PyDict_SetItemString(py_dict, "rid", PyLong_FromLong(entry->rid));
			PyDict_SetItemString(py_dict, "acct_flags", PyLong_FromLong(entry->acct_flags));
...
static PyObject *py_pdb_get_trusteddom_pw(PyObject *self, PyObject *args)
{
	...
	PyDict_SetItemString(py_value, "last_set_tim", PyLong_FromLong(last_set_time));
...
static PyObject *py_pdb_get_secret(PyObject *self, PyObject *args)
{
	...
	PyDict_SetItemString(py_secret, "secret_current",
			PyStr_FromStringAndSize((char *)secret_current.data, secret_current.length));
	...
	PyDict_SetItemString(py_secret, "secret_old",
			PyStr_FromStringAndSize((char *)secret_old.data, secret_old.length));
...
static PyObject *py_pdb_set_secret(PyObject *self, PyObject *args)
{
	...
	PY_CHECK_TYPE(&PyStr_Type, py_secret_cur, return NULL;);
	PY_CHECK_TYPE(&PyStr_Type, py_secret_old, return NULL;);
